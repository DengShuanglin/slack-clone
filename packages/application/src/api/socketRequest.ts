...
export function useAddFriendRequest() {
    ...
    if (ctx.socketClient != null) {
        return manualFactory<AddFriendRequestType, SocketDataEntity>(ctx.socketClient)('addFriend');
    } else {
        return undefined;
    }
}
...
type ConfirmFriendRequestType = { friend_id: string, nickname: string, avatar: string }
...
export function useConfirmFriendListener() {
    ...
    useEffect(() => {
        ctx.eventCallback['confirmFriend'] = {
            ...ctx.eventCallback['confirmFriend'],
            "response": (data: SocketDataEntity<ConfirmFriendRequestType>, fn: Function) => {
                ref[0] = data;
...
export function useJoinFriendSocketRequest() {
    const ctx = useContext(SocketHubContext);
    if (ctx.socketClient != null) {
        return manualFactory<JoinFriendSocketRequestType, SocketDataEntity<JoinFriendSocketResponseType>>(ctx.socketClient)('joinFriendSocket');
    } else {
        return undefined;
    }
}
...
export function useFriendMessageRequest() {
    const ctx = useContext(SocketHubContext);
    if (ctx.socketClient != null) {
        return manualFactory<FriendMessage, SocketDataEntity<FriendMessage & {time:string}>>(ctx.socketClient)('friendMessage');
    } else {
        return undefined;
    }
}
export function useFriendMessageListener() {
    ...
    useEffect(() => {
        ctx.eventCallback['friendMessage'] = {
            ...ctx.eventCallback['friendMessage'],
            "response": (data: SocketDataEntity<FriendMessage & {time:string}>, fn: Function) => {
                ref[0] = data;
